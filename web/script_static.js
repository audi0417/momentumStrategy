...
class MomentumDashboardStatic {
    ...
    setupEventListeners() {
        ...
        if (notificationBtn) {
            notificationBtn.addEventListener('click', () => {
                this.showToast('這是靜態演示版本，數據為示例數據', 'info');
            });
        }
    }
    ...
    getStockMomentumData(stockId) {
        const dates = Object.keys(stockData.dates).sort().reverse();
        const result = [];
        
        dates.forEach(date => {
            const stockInfo = stockData.dates[date][stockId];
            if (stockInfo) {
                result.push({
                    date: date,
    ...
    updateMarketOverview() {
        const dates = Object.keys(stockData.dates);
        const latestDate = dates.sort().reverse()[0];
        const latestData = stockData.dates[latestDate];
...
document.addEventListener('DOMContentLoaded', () => {
    window.dashboard = new MomentumDashboardStatic();
});

// Export for potential use in other modules
if (typeof module !== 'undefined' && module.exports) {
    module.exports = MomentumDashboardStatic;
}